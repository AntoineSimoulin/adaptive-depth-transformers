...
import os
import time
from albert import classifier_utils, tags_utils
...
flags.DEFINE_bool("do_eval_on_ptb", False, "Whether to run eval on the penn tree bank.")
...
def main(_):
  ...
  if FLAGS.do_eval_on_ptb:
    ptb_processor = tags_utils.ConllProcessor(
      use_spm=True if FLAGS.spm_model_file else False,
      do_lower_case=FLAGS.do_lower_case)

    ptb_examples = ptb_processor.get_all_examples(FLAGS.data_dir)
    num_actual_ptb_examples = len(ptb_examples)

    if FLAGS.use_tpu:
      while len(ptb_examples) % FLAGS.eval_batch_size != 0:
        ptb_examples.append(classifier_utils.PaddingInputExample())

    predict_file = os.path.join(FLAGS.output_dir, "predict_pdb.tf_record")
    label_list = ptb_processor.get_labels()
    predict_drop_remainder = True if FLAGS.use_tpu else False

    eval_ptb_input_fn = classifier_utils.file_based_input_fn_builder(
    ...
    results = estimator.predict(
      input_fn=eval_ptb_input_fn,
      checkpoint_path=checkpoint_path)

    n_updates = np.stack([prediction['n_updates'] for prediction in results], axis=1)
    upos = np.stack([prediction['upos_ids'] for prediction in results], axis=1)
    deps = np.stack([prediction['deps_ids'] for prediction in results], axis=1)
